...
document.querySelector("#start_chat").addEventListener("click", (event) => {
  ...
  socket.on('admin_send_to_client', message => {
    ...
    template_admin.scrollTo(0, divMessages.scrollHeight)
  })
...
document.querySelector('#send_message_button').addEventListener('click', (event) => {
  ...
  const template_clinet = document.getElementById('message-user-template').innerHTML

  const rendered = Mustache.render(template_clinet, {
    message: text.value,
    email: emailUser
  })
